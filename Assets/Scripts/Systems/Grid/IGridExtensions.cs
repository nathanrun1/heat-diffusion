...
namespace Blizzard.Grid
{
    public static class IGridExtensions
    {
        ...
        /// <summary>
        /// Writes subgrid defined by given grid bounds into given subgrid
        /// </summary>
        public static void WriteToSubgrid<T>(this IGrid<T> grid, IGrid<T> subgrid, Vector2Int offset) where T : struct
        {
            for (int y = offset.y; y < offset.y + subgrid.Height; ++y)
            {
                for (int x = offset.x; x < offset.x + subgrid.Width; ++x)
                {
                    int subgridIndex = (y - offset.y) * subgrid.Width + (x - offset.x);
                    subgrid.SetAt(x, y, grid.GetAt(x, y));
                }
            }
        }

        /// <summary>
        /// Reads given subgrid into this grid in area defined by given gridbounds
        /// </summary>
        public static void ReadFromSubgrid<T>(this IGrid<T> grid, IGrid<T> subgrid, Vector2Int offset) where T : struct
        {
            for (int y = offset.y; y < offset.y + subgrid.Height; ++y)
            {
                for (int x = offset.x; x < offset.x + subgrid.Width; ++x)
                {
                    int subgridIndex = (y - offset.y) * subgrid.Width + (x - offset.x);
                    grid.SetAt(x, y, subgrid.GetAt(x, y));
                }
            }
        }
    }
